import { setSelectedBoardId } from '../../store';
...
export const SelectBoard = () => {
	...
	return (
		<div className='headingM flex flex-col items-start justify-around h-full p-5'>
			...
			{boards &&
				boards.map(board => (
					<button className='text-MediumGrey' key={board.boardId} onClick={() => dispatch(setSelectedBoardId(board.boardId))}>
						{board.name}
					</button>
				))}
			<button className='text-MainPurple'>+Create New Board</button>
